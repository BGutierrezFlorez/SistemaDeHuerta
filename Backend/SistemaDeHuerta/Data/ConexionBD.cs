...
namespace SistemaDeHuerta
{
    public class ConexionBD
    {
        ...
        public bool Consultar(SqlCommand cmd, bool conParametros)
        {
            if (cmd == null)
            {
                strError = "El comando no puede ser nulo";
                return false;
            }
            ...
            try
            {
                cmd.Connection = objCnnBD;
                cmd.CommandType = conParametros ? CommandType.StoredProcedure : CommandType.Text;
                objReader = cmd.ExecuteReader();
                return true;
            }
            catch (Exception ex)
            {
                strError = "Falla en ejecutar comando con SqlCommand - " + ex.Message;
                return false;
            }
        }
        ...
        public bool EjecutarSentencia(string SentenciaSQL, bool blnCon_Parametros)
        {
            ...
            try
            {
                int affectedRows = objCmdBD.ExecuteNonQuery();
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool EjecutarComando(SqlCommand cmd)
        {
            try
            {
                if (!this.AbrirConexion()) // Verifica que la conexión se abra correctamente
                {
                    strError = "No se pudo abrir la conexión a la base de datos.";
                    return false;
                }

                if (objCnnBD == null) // Protección adicional por si acaso
                {
                    strError = "La conexión está nula incluso después de abrirla.";
                    return false;
                }
            ...
            {
                strError = "Error al ejecutar comando - " + ex.Message;
                return false;
            }
            finally
            {
